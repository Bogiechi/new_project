...
class Router:
    def __init__(self):
        self._mysql_vtuberdata_conn = (
            clients.get_mysql_vtuberdata_conn()
        )

    def check_mysql_vtuberdata_conn_alive(
        self,
    ):
        self._mysql_vtuberdata_conn = check_connect_alive(
            self._mysql_vtuberdata_conn,
            clients.get_mysql_vtuberdata_conn,
        )
        return (
    ...
    @property
    def mysql_vtuberdata_conn(self):
        return (
            self.check_mysql_vtuberdata_conn_alive()
        )

    def close_connection(self):
        self._mysql_vtuberdata_conn.close()